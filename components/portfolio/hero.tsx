  ...
  return (
    <section className="relative flex min-h-[100svh] w-full items-center overflow-hidden px-5 py-24 sm:px-10 lg:px-16">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        {/* Blue glows — dark & light mode */}
        <div className="absolute -top-40 left-1/4 h-[600px] w-[600px] -translate-x-1/2 rounded-full
          bg-blue-500/20 dark:bg-blue-500/15 blur-[130px]" />
        <div className="absolute bottom-0 right-1/4 h-[500px] w-[500px] rounded-full
          bg-blue-400/15 dark:bg-blue-400/10 blur-[110px]" />
        <div className="absolute left-0 top-1/2 h-[350px] w-[350px] -translate-y-1/2 rounded-full
          bg-indigo-500/12 dark:bg-indigo-500/8 blur-[100px]" />
        <div className="absolute right-0 top-0 h-[300px] w-[300px] rounded-full
          bg-blue-300/10 dark:bg-blue-300/6 blur-[90px]" />

        {/* Decorative corner shapes */}
        <div className="absolute left-8 top-32 h-24 w-24 rounded-full border border-blue-400/20 dark:border-blue-400/15" />
        <div className="absolute left-16 top-40 h-12 w-12 rounded-full border border-blue-400/15 dark:border-blue-400/10" />
        <div className="absolute bottom-32 right-12 h-32 w-32 rounded-2xl border border-primary/15 dark:border-primary/10 rotate-12" />
        <div className="absolute bottom-48 right-20 h-16 w-16 rounded-full border border-blue-300/20 dark:border-blue-300/10 rotate-45" />
        ...
        {/* Grid lines top-right decorative */}
        <svg className="absolute right-0 top-0 h-64 w-64 opacity-[0.06] dark:opacity-[0.04]" viewBox="0 0 200 200">
          {[0,40,80,120,160,200].map(x => (
            <line key={`v${x}`} x1={x} y1="0" x2={x} y2="200" stroke="currentColor" strokeWidth="0.5"/>
          ))}
          {[0,40,80,120,160,200].map(y => (
            <line key={`h${y}`} x1="0" y1={y} x2="200" y2={y} stroke="currentColor" strokeWidth="0.5"/>
          ))}
        </svg>
      </div>
      ...
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-10 lg:grid-cols-[1.3fr_1fr]">
        ...
        <div className="flex flex-col">
          <motion.span
            ...
            className="font-marker text-3xl text-primary sm:text-4xl"
          >
          ...
          <h1 className="mt-2 font-display leading-[0.86] tracking-tight">
            <motion.span
              ...
              animate={showName ? { opacity: 1, y: 0, skewY: 0 } : {}}
          ...
          <span className="mt-1 font-marker text-2xl text-foreground/80 sm:text-3xl">
        ...
        >
          {/* Blue glow behind photo - stronger in both modes */}
          <div className="absolute inset-0 rounded-3xl bg-blue-500/25 dark:bg-blue-500/20 blur-3xl" />
          <div className="absolute -inset-4 rounded-3xl bg-blue-400/10 dark:bg-blue-400/8 blur-2xl" />
          ...
          <div className="animate-floaty relative h-full w-full overflow-hidden rounded-3xl border border-foreground/10 shadow-2xl shadow-blue-500/25 dark:shadow-blue-500/15">
            <Image
