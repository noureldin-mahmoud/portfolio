  ...
  return (
    <section className="relative flex min-h-[100svh] w-full items-center overflow-hidden px-5 py-24 sm:px-10 lg:px-16">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        ...
        {/* Blue ambient glows */}
        <div className="absolute -top-32 left-1/4 h-[500px] w-[500px] -translate-x-1/2 rounded-full bg-blue-600/10 blur-[120px]" />
        <div className="absolute bottom-0 right-1/4 h-[400px] w-[400px] rounded-full bg-blue-500/8 blur-[100px]" />
        <div className="absolute top-1/2 left-0 h-[300px] w-[300px] -translate-y-1/2 rounded-full bg-indigo-600/8 blur-[90px]" />
        
        {/* Floating particles */}
        {[...Array(6)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute size-1 rounded-full bg-blue-400/40"
            style={{
              left: `${15 + i * 14}%`,
              top: `${20 + (i % 3) * 25}%`,
            }}
      ...
      <div className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-10 lg:grid-cols-[1.3fr_1fr]">
        ...
        <div className="flex flex-col">
          ...
          >
            ...
            >
              <span className={`size-2 rounded-full ${profile.available ? 'animate-pulse bg-green-500' : 'bg-amber-500'}`} />
              {profile.available ? 'Available' : profile.availabilityNote}
            </motion.span>
            <motion.span
              whileHover={{ scale: 1.05 }}
              className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-medium shadow-sm"
        ...
        >
          ...
          <div className="absolute inset-0 rounded-3xl bg-blue-500/15 blur-2xl" />
          ...
          <div className="animate-floaty relative h-full w-full overflow-hidden rounded-3xl border border-foreground/10 shadow-2xl shadow-blue-500/20">
            <Image
          ...
          <motion.div
            ...
            className="absolute -right-4 -top-4 grid size-10 place-items-center rounded-xl border border-blue-400/30 bg-card shadow-lg shadow-blue-500/10"
          >
